...
raw_data_sample <- raw_data_sample %>% replace_with_na(replace = list(Arsenophonus=40.00000))
...
bc <- boxcox(normalized_perSample$Arsenophonus ~ normalized_perSample$Group)
bc_nn <- boxcox(not_normalized_perSample$Arsenophonus ~ not_normalized_perSample$Group)
...
normalized_perSample <- mutate(normalized_perSample, bcArsenophonus = powerTransform(normalized_perSample$Arsenophonus, lambda))
not_normalized_perSample <- mutate(not_normalized_perSample, bcArsenophonus = powerTransform(not_normalized_perSample$Arsenophonus, lambda_nn))

#ANOVA model using normalized, transformed data
ANOVA_model_norm <- aov(normalized_perSample$bcArsenophonus~Group, data=normalized_perSample)
par(mfrow = c(2, 2))
...
ANOVA_model_nn <- aov(not_normalized_perSample$bcArsenophonus~Group, data=not_normalized_perSample)
par(mfrow = c(2, 2))
plot(ANOVA_model_nn)
...
normalized_perSample <- mutate(normalized_perSample, OsArsenophonus = normalized_perSample$Arsenophonus)
not_normalized_perSample <- mutate(not_normalized_perSample, OsArsenophonus = not_normalized_perSample$Arsenophonus)
...
p1<-ggplot(normalized_perSample, aes(x=Group, y=bcArsenophonus)) + 
  ...
  ggtitle("Arsenophonus", subtitle = paste("Actin normalized, transformed scale, Box Cox lambda = ", round(lambda, digits=3))) +
...
p1
#ggsave(filename = "Arsenophonus-AN-Ts.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


#Box plot Actin normalized data with Tukey letters, on original scale
p2<-ggplot(normalized_perSample, aes(x=Group, y=OsArsenophonus)) + 
  geom_boxplot(show.legend = FALSE , aes(fill=sex)) + scale_fill_manual(values=c("#bdbdbd", "#636363")) + 
  geom_text(data = generate_label_df(normalized_perSample, tHSD, 'Group', 9), aes(x = plot.labels, y = V1, label = labels)) +
  ggtitle("Arsenophonus", subtitle = paste("Actin normalized, original scale, Box Cox lambda = ", round(lambda, digits=3))) +
  ...
  scale_x_discrete(labels = str_wrap(c("Antibiotic female", "Antibiotic male", "Control female", "Control male", "Colony female", "Colony male"), width = 10))
p2
#ggsave(filename = "Arsenophonus-AN-Os.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


#Box plot NOT normalized data with Tukey letters, on transformed scale
p3<-ggplot(not_normalized_perSample, aes(x=Group, y=bcArsenophonus)) + 
  geom_boxplot(show.legend = FALSE , aes(fill=sex)) + scale_fill_manual(values=c("#bdbdbd", "#636363")) + 
  geom_text(data = generate_label_df(not_normalized_perSample, tHSD_nn, 'Group', 8), aes(x = plot.labels, y = V1, label = labels)) +
  ggtitle("Arsenophonus", subtitle = paste("Not normalized, transformed scale, Box Cox lambda = ", round(lambda_nn, digits=3))) +
  xlab("Treatments") + ylab("Relative quantity (transformed)") +
  theme(axis.text = element_text(size = 11)) +
  theme(axis.title = element_text(size = 12)) +
  theme(plot.title = element_text(face="bold")) +
  scale_x_discrete(labels = str_wrap(c("Antibiotic female", "Antibiotic male", "Control female", "Control male", "Colony female", "Colony male"), width = 10))
p3
#ggsave(filename = "Arsenophonus-NN-Ts.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


#Box plot NOT normalized data with Tukey letters, on original scale
p4<-ggplot(not_normalized_perSample, aes(x=Group, y=OsArsenophonus)) + 
  geom_boxplot(show.legend = FALSE , aes(fill=sex)) + scale_fill_manual(values=c("#bdbdbd", "#636363")) + 
  geom_text(data = generate_label_df(not_normalized_perSample, tHSD_nn, 'Group', 9), aes(x = plot.labels, y = V1, label = labels)) +
  ggtitle("Arsenophonus", subtitle = paste("Not normalized, original scale, Box Cox lambda = ", round(lambda_nn, digits=3))) +
  xlab("Treatments") + ylab("Relative quantity") +
  theme(axis.text = element_text(size = 11)) +
  theme(axis.title = element_text(size = 12)) +
  theme(plot.title = element_text(face="bold")) +
...
p4
#ggsave(filename = "Arsenophonus-NN-Os.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


ggsave(filename = "Arsenophonus-Ts.tiff", path="Paper/Graphs", arrangeGrob(p3, p1,nrow = 1), width = 322, height = 185, units="mm", device='tiff', dpi=300, compression = "lzw")
ggsave(filename = "Arsenophonus-Os.tiff", path="Paper/Graphs", arrangeGrob(p4, p2,nrow = 1), width = 322, height = 185, units="mm", device='tiff', dpi=300, compression = "lzw")

write.csv2(not_normalized_perSample,'Not normalized per sampleA.csv')
write.csv2(normalized_perSample,'Normalized per sampleA.csv')


no<-aggregate(Arsenophonus ~ Group, data=normalized_perSample, function(x) {sum(is.na(x))}, na.action = NULL)
yes<-aggregate(cbind(count = Arsenophonus) ~ Group, data = normalized_perSample, FUN = function(x){NROW(x)})
Ars_counts <- cbind(yes, no[c("Arsenophonus")])
Ars_counts
names(Ars_counts)[1] <- "Treatment"
names(Ars_counts)[2] <- "Positive"
names(Ars_counts)[3] <- "Negative"
Ars_counts

#Sum up the totals, calculate percentage for easy interpretation and export
Ars_counts_export <- mutate(Ars_counts, Total = Ars_counts$Positive + Ars_counts$Negative)
Ars_counts_export <- mutate(Ars_counts_export, PercentNegative = Ars_counts_export$Negative / Ars_counts_export$Total)
write.csv2(Ars_counts_export,'Arsenophonus-presence.csv')
...
Ars_counts <- matrix.please(Ars_counts)
mosaic(Ars_counts, shade=TRUE, legend=TRUE, labeling= labeling_border(rot_labels = c(0,0,0,0), 
                                                                      just_labels = c("center", 
                                                                                      "center", 
                                                                                      "center", 
                                                                                      "right"), set_varnames = list(A = "", B = "qPCR result")))
library(rcompanion)
PT = pairwiseNominalIndependence(Ars_counts,
                                 fisher = TRUE,
                                 gtest  = FALSE,
                                 chisq  = FALSE,
                                 digits = 3,
...
PT
#write.csv2(PT,'Arsenophonus-presence-Fisher.csv')
