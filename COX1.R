...
raw_data_sample <- raw_data_sample %>% replace_with_na(replace = list(COX1=40.00000))
...
bc <- boxcox(normalized_perSample$COX1 ~ normalized_perSample$Group)
bc_nn <- boxcox(not_normalized_perSample$COX1 ~ not_normalized_perSample$Group)
...
normalized_perSample <- mutate(normalized_perSample, bcCOX1 = powerTransform(normalized_perSample$COX1, lambda))
not_normalized_perSample <- mutate(not_normalized_perSample, bcCOX1 = powerTransform(not_normalized_perSample$COX1, lambda_nn))

#ANOVA model using normalized, transformed data
ANOVA_model_norm <- aov(normalized_perSample$bcCOX1~Group, data=normalized_perSample)
par(mfrow = c(2, 2))
...
ANOVA_model_nn <- aov(not_normalized_perSample$bcCOX1~Group, data=not_normalized_perSample)
par(mfrow = c(2, 2))
plot(ANOVA_model_nn)
...
normalized_perSample <- mutate(normalized_perSample, OsCOX1 = normalized_perSample$COX1)
not_normalized_perSample <- mutate(not_normalized_perSample, OsCOX1 = not_normalized_perSample$COX1)
...
p1<-ggplot(normalized_perSample, aes(x=Group, y=bcCOX1)) + 
  ...
  ggtitle("COX1", subtitle = paste("Actin normalized, transformed scale, Box Cox lambda = ", round(lambda, digits=3))) +
...
p1
#ggsave(filename = "COX1-AN-Ts.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


#Box plot Actin normalized data with Tukey letters, on original scale
p2<-ggplot(normalized_perSample, aes(x=Group, y=OsCOX1)) + 
  geom_boxplot(show.legend = FALSE , aes(fill=sex)) + scale_fill_manual(values=c("#bdbdbd", "#636363")) + 
  geom_text(data = generate_label_df(normalized_perSample, tHSD, 'Group', 9), aes(x = plot.labels, y = V1, label = labels)) +
  ggtitle("COX1", subtitle = paste("Actin normalized, original scale, Box Cox lambda = ", round(lambda, digits=3))) +
  ...
  scale_x_discrete(labels = str_wrap(c("Antibiotic female", "Antibiotic male", "Control female", "Control male", "Colony female", "Colony male"), width = 10))
p2
#ggsave(filename = "COX1-AN-Os.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


#Box plot NOT normalized data with Tukey letters, on transformed scale
p3<-ggplot(not_normalized_perSample, aes(x=Group, y=bcCOX1)) + 
  geom_boxplot(show.legend = FALSE , aes(fill=sex)) + scale_fill_manual(values=c("#bdbdbd", "#636363")) + 
  geom_text(data = generate_label_df(not_normalized_perSample, tHSD_nn, 'Group', 8), aes(x = plot.labels, y = V1, label = labels)) +
  ggtitle("COX1", subtitle = paste("Not normalized, transformed scale, Box Cox lambda = ", round(lambda_nn, digits=3))) +
  xlab("Treatments") + ylab("Relative quantity (transformed)") +
  theme(axis.text = element_text(size = 11)) +
  theme(axis.title = element_text(size = 12)) +
  theme(plot.title = element_text(face="bold")) +
  scale_x_discrete(labels = str_wrap(c("Antibiotic female", "Antibiotic male", "Control female", "Control male", "Colony female", "Colony male"), width = 10))
p3
#ggsave(filename = "COX1-NN-Ts.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


#Box plot NOT normalized data with Tukey letters, on original scale
p4<-ggplot(not_normalized_perSample, aes(x=Group, y=OsCOX1)) + 
  geom_boxplot(show.legend = FALSE , aes(fill=sex)) + scale_fill_manual(values=c("#bdbdbd", "#636363")) +  
  geom_text(data = generate_label_df(not_normalized_perSample, tHSD_nn, 'Group', 9), aes(x = plot.labels, y = V1, label = labels)) +
  ggtitle("COX1", subtitle = paste("Not normalized, original scale, Box Cox lambda = ", round(lambda_nn, digits=3))) +
  xlab("Treatments") + ylab("Relative quantity") +
  theme(axis.text = element_text(size = 11)) +
  theme(axis.title = element_text(size = 12)) +
  theme(plot.title = element_text(face="bold")) +
  scale_x_discrete(labels = str_wrap(c("Antibiotic female", "Antibiotic male", "Control female", "Control male", "Colony female", "Colony male"), width = 10))
p4
#ggsave(filename = "COX1-NN-Os.tiff", path="Paper/original scale", width = 600, height = 700, units="px", device='tiff', dpi=96)


ggsave(filename = "COX1-Ts.tiff", path="Paper/Graphs", arrangeGrob(p3, p1,nrow = 1), width = 322, height = 185, units="mm", device='tiff', dpi=300, compression = "lzw")
ggsave(filename = "COX1-Os.tiff", path="Paper/Graphs", arrangeGrob(p4, p2,nrow = 1), width = 322, height = 185, units="mm", device='tiff', dpi=300, compression = "lzw")
